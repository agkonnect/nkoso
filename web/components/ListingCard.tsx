...
export default function ListingCard({ listing, onReact }: ListingCardProps) {
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-dark-700">
          <div className="flex items-center gap-1">
            <button
              onClick={() => onReact?.(listing.id, 'hype')}
            ...
            <button
              onClick={() => onReact?.(listing.id, 'can_help')}
            ...
            <button
              onClick={() => onReact?.(listing.id, 'would_invest')}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg hover:bg-dark-700 text-gray-400 hover:text-brand-400 transition-all text-xs">
              <DollarSign className="w-4 h-4" />
              <span>{formatNumber(listing.reactions_count?.would_invest ?? 0)}</span>
            </button>
            <button
              onClick={() => onReact?.(listing.id, 'support')}
